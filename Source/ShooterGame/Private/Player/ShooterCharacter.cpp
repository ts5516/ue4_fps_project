...
void AShooterCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Dash", IE_Pressed, this, &AShooterCharacter::OnDashing);
...
void AShooterCharacter::OnDashing()
{
	if (IsTargeting() || GetCharacterMovement()->IsFalling()|| GetWorldTimerManager().IsTimerActive(DashTimerHandle))
	{
		return;
	}
	const FVector ForwardDir = this->GetActorRotation().Vector();
	LaunchCharacter(ForwardDir * DashDistance, true, true);
	GetWorld()->GetTimerManager().SetTimer(DashTimerHandle, this, &AShooterCharacter::DashTimer, DashCoolDownTime, false);
}

void AShooterCharacter::DashTimer()
{
	return;
}
